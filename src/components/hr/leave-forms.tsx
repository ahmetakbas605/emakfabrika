...
export function SetEntitlementForm({ departmentId, employees, currentYear }: { departmentId: string; employees: { id: string; firstName: string; lastName: string }[]; currentYear: number }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Yıl</label><input name="year" type="number" defaultValue={currentYear} style={{ padding: 6, width: 90 }} /></div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>İzin Türü</label>
        <select name="leaveType" required style={{ padding: 6, width: 170 }}>
          {Object.entries(LEAVE_TYPE_LABELS).map(([code, label]) => <option key={code} value={code}>{label}</option>)}
        </select>
      </div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Hak Ediş (gün)</label><input name="entitlementDays" type="number" step="0.5" min={0} style={{ padding: 6, width: 90 }} /></div>

...
export function CompensationForm({ departmentId, employeeId, currencies }: { departmentId: string; employeeId: string; currencies: { code: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Yürürlük Tarihi</label><input name="effectiveDate" type="date" required style={{ padding: 6 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Maaş</label><input name="baseSalary" type="number" step="0.01" min={0} required style={{ padding: 6, width: 120 }} /></div>
      <div>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Değişiklik Nedeni</label><input name="changeReason" style={{ padding: 6, width: 160 }} placeholder="Zam / Terfi / İşe Giriş" /></div>

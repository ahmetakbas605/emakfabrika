...
export function BonusForm({ departmentId, employeeId, currencies }: { departmentId: string; employeeId: string; currencies: { code: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Tutar</label><input name="amount" type="number" step="0.01" min={0} required style={{ padding: 6, width: 120 }} /></div>
      <div>
      ...
      <div style={{ width: '100%' }}><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Gerekçe</label><textarea name="reason" rows={2} style={{ padding: 6, width: '100%' }} /></div>

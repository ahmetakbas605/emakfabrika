...
export function RecordInspectionForm({ incomingSources, processSources, finalSources, products }: {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Kaynak Kayıt</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Ürün (ops.)</label>
        <select name="productId" style={{ padding: 6, minWidth: 140 }}>
          <option value="">Seçin</option>
          {products.map((p) => <option key={p.id} value={p.id}>{p.sku} — {p.name}</option>)}
        </select>
      </div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Muayene Edilen</label><input name="inspectedQty" type="number" step="0.01" required style={{ padding: 6, width: 90 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Geçen</label><input name="passedQty" type="number" step="0.01" required style={{ padding: 6, width: 90 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Ret</label><input name="failedQty" type="number" step="0.01" required style={{ padding: 6, width: 90 }} /></div>
      <div>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Not (ops.)</label><input name="notes" style={{ padding: 6, width: 160 }} /></div>
...
export function CreateNcrForm({ failedInspections, suppliers, products }: {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>İlgili Muayene (ops.)</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Tedarikçi (ops.)</label>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Başlık</label><input name="title" required style={{ padding: 6, width: 200 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Açıklama</label><input name="description" required style={{ padding: 6, width: 260 }} /></div>
      <div>
...
export function RecordNcrRootCauseForm({ ncrId }: { ncrId: string }) {
  ...
  return (
    <form action={formAction} style={{ border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <input type="hidden" name="ncrId" value={ncrId} />
      <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)', marginBottom: 4 }}>Kök Neden</label>
...
export function RecordNcrActionsForm({ ncrId }: { ncrId: string }) {
  ...
  return (
    <form action={formAction} style={{ border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <input type="hidden" name="ncrId" value={ncrId} />
      <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)', marginBottom: 4 }}>Düzeltici Faaliyet</label>
      <textarea name="correctiveAction" required rows={2} style={{ width: '100%', padding: 6, marginBottom: 8 }} />
      <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)', marginBottom: 4 }}>Önleyici Faaliyet</label>

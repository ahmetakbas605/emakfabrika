...
export function CreateMachineForm({ workCenters }: { workCenters: { id: string; code: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>İdeal Çevrim Süresi (sn/adet, ops.)</label><input name="idealCycleTimeSeconds" type="number" step="0.01" style={{ padding: 6, width: 110 }} /></div>
...
export function StartDowntimeForm({ machines, reasons }: { machines: { id: string; code: string; name: string }[]; reasons: { code: string; name: string; category: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Makine</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Neden</label>

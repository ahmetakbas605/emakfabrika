...
export function PartyForm({ currencies, paymentTerms }: { currencies: { code: string; name: string }[]; paymentTerms: { id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ border: '1px solid #ddd', padding: 16, borderRadius: 6, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Ticari Ad</label>
        <input name="tradeName" style={{ padding: 6, width: '100%' }} />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Kod (boşsa otomatik)</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>VKN/TCKN</label>
        <input name="taxNumber" style={{ padding: 6, width: '100%' }} />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Vergi Dairesi</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Ödeme Vadesi</label>

...
export function ProductBarcodeForm({ productId }: { productId: string }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Barkod</label>
...
export function ProductSupplierForm({ productId, suppliers, currencies }: { productId: string; suppliers: { id: string; legalName: string }[]; currencies: { code: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Tedarikçi SKU</label>
        <input name="supplierSku" style={{ padding: 6, width: 120 }} />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Alış Fiyatı</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Teslim Süresi (gün)</label>
        <input name="leadTimeDays" type="number" min="0" style={{ padding: 6, width: 80 }} />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Min. Sipariş Miktarı</label>

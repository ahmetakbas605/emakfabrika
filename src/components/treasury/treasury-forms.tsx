...
export function CreateCashFlowItemForm({ currencies }: { currencies: { code: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Açıklama</label><input name="description" required style={{ padding: 6, width: 200 }} /></div>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Beklenen Tarih</label><input name="expectedDate" type="date" required style={{ padding: 6 }} /></div>

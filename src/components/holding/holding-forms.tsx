...
export function CreateHoldingForm() {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Yeni Holding Adı</label>
      ...
      {state?.error ? <p style={{ color: 'var(--dim-danger)', fontSize: 13, marginLeft: 8 }}>{state.error}</p> : null}
      {state?.success ? <p style={{ color: 'var(--dim-success)', fontSize: 13, marginLeft: 8 }}>{state.success}</p> : null}
    </form>
  );
}
...
export function MoveCompanyForm({ companies, holdings }: { companies: { id: string; name: string; holdingId: string | null }[]; holdings: { id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Şirket</label>
        <select name="companyId" required style={{ padding: 6, width: 220 }}>
          {companies.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Hedef Holding</label>

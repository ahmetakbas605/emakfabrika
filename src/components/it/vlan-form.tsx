...
export function VlanForm({ departmentId }: { departmentId: string }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>VLAN No</label><input name="vlanNumber" type="number" min={1} required style={{ padding: 6, width: 90 }} /></div>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Amaç</label><input name="purpose" placeholder="ör. Sunucular" style={{ padding: 6 }} /></div>

...
export function KbArticleForm({ departmentId, categories }: { departmentId: string; categories: { id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', flexDirection: 'column', gap: 8, border: '1px solid #ddd', padding: 12, borderRadius: 6, maxWidth: 560 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Kategori (opsiyonel)</label>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>İçerik</label><textarea name="content" required rows={5} style={{ padding: 6, width: '100%' }} /></div>

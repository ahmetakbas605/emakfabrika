...
export function AssignIpForm({ departmentId, subnetId, prefillIp, assets }: { departmentId: string; subnetId: string; prefillIp?: string; assets: { id: string; assetTag: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>IP Adresi</label><input name="ipAddress" defaultValue={prefillIp} required style={{ padding: 6, width: 140 }} /></div>
      <div>

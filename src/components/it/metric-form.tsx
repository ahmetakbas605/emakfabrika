...
export function MetricForm({ departmentId, targets }: { departmentId: string; targets: { id: string; assetTag: string; targetType: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Metrik Adı</label><input name="metricName" placeholder="up / cpu_percent / latency_ms" required style={{ padding: 6, width: 160 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Değer</label><input name="value" type="number" step="0.01" required style={{ padding: 6, width: 90 }} /></div>

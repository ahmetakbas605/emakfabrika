...
export function SubnetForm({ departmentId, vlans }: { departmentId: string; vlans: { id: string; vlanNumber: number; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>CIDR</label><input name="cidr" placeholder="192.168.1.0/24" required style={{ padding: 6, width: 160 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Gateway</label><input name="gateway" placeholder="192.168.1.1" style={{ padding: 6 }} /></div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>VLAN</label>
        <select name="vlanId" style={{ padding: 6 }}>
          <option value="">Seçilmedi</option>
          {vlans.map((v) => <option key={v.id} value={v.id}>{v.vlanNumber} — {v.name}</option>)}
        </select>
      </div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Açıklama</label><input name="description" style={{ padding: 6 }} /></div>

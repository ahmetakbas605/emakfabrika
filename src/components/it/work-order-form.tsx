...
export function WorkOrderForm({ departmentId, tickets }: { departmentId: string; tickets: { id: string; ticketNo: string; title: string }[] }) {
  ...
  if (tickets.length === 0) return <p style={{ color: 'var(--dim-slate)', fontSize: 13 }}>Work order açılabilecek (saha işi tipinde, henüz açılmamış) bir ticket yok.</p>;
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Saha İşi Ticket</label>

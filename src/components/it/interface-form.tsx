...
export function InterfaceForm({ departmentId, assets, vlans }: { departmentId: string; assets: { id: string; assetTag: string; name: string }[]; vlans: { id: string; vlanNumber: number; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Arayüz Adı</label><input name="name" placeholder="eth0" required style={{ padding: 6, width: 100 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>MAC</label><input name="macAddress" placeholder="AA:BB:CC:DD:EE:FF" style={{ padding: 6, width: 160 }} /></div>
      <div>

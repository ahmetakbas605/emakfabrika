...
export function BackupJobForm({ departmentId, assets }: { departmentId: string; assets: { id: string; assetTag: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Kaynak</label><input name="source" required style={{ padding: 6, width: 140 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Hedef</label><input name="destination" required style={{ padding: 6, width: 140 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Zamanlama</label><input name="schedule" placeholder="ör. Her gece 02:00" style={{ padding: 6, width: 140 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Saklama (gün)</label><input name="retentionDays" type="number" min={1} defaultValue={30} style={{ padding: 6, width: 90 }} /></div>

...
export function StockItemForm({ departmentId, accounts, products }: { departmentId: string; accounts: { id: string; code: string; name: string }[]; products: { id: string; sku: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Stok Değer Hesabı (opsiyonel)</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Master Ürün (opsiyonel)</label>

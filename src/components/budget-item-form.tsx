...
export function BudgetItemForm({ departmentId, budgetId, accounts }: { departmentId: string; budgetId: string; accounts: { code: string; id: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Hesap</label>
      ...
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Ay (opsiyonel, boşsa yıllık)</label>
        <input name="month" type="number" min={1} max={12} style={{ padding: 6, width: 80 }} />
      </div>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Planlanan Tutar</label>

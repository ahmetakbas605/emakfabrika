...
export function CreateLeadForm() {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 12, borderRadius: 6 }}>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>İletişim Adı</label><input name="contactName" required style={{ padding: 6, width: 160 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Şirket</label><input name="companyName" style={{ padding: 6, width: 160 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>E-posta</label><input name="email" type="email" style={{ padding: 6, width: 160 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Telefon</label><input name="phone" style={{ padding: 6, width: 130 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: 'var(--dim-on-surface-variant)' }}>Kaynak</label><input name="source" placeholder="Web, Referans, Fuar..." style={{ padding: 6, width: 130 }} /></div>
...
export function ConvertLeadForm({ leadId, parties }: { leadId: string; parties: { id: string; legalName: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 6, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid var(--dim-border-soft)', padding: 10, borderRadius: 4, marginTop: 6 }}>
      <input type="hidden" name="leadId" value={leadId} />
      <div><label style={{ display: 'block', fontSize: 11, color: 'var(--dim-on-surface-variant)' }}>Fırsat Adı</label><input name="opportunityName" required style={{ padding: 5, width: 150 }} /></div>
      <div><label style={{ display: 'block', fontSize: 11, color: 'var(--dim-on-surface-variant)' }}>Tahmini Değer</label><input name="estimatedValue" type="number" step="0.01" style={{ padding: 5, width: 90 }} /></div>
      <div><label style={{ display: 'block', fontSize: 11, color: 'var(--dim-on-surface-variant)' }}>Para Birimi</label><input name="currencyCode" defaultValue="TRY" style={{ padding: 5, width: 60 }} /></div>
      <div><label style={{ display: 'block', fontSize: 11, color: 'var(--dim-on-surface-variant)' }}>Beklenen Kapanış</label><input name="expectedCloseDate" type="date" style={{ padding: 5 }} /></div>
      <div>
        <label style={{ display: 'block', fontSize: 11, color: 'var(--dim-on-surface-variant)' }}>Mevcut Cariye Eşleştir (opsiyonel)</label>

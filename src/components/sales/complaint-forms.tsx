...
export function CreateComplaintForm({ parties }: { parties: { id: string; legalName: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Konu</label><input name="subject" required style={{ padding: 6, width: 200 }} /></div>
      <div>
      ...
      <div style={{ width: '100%' }}><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Açıklama</label><textarea name="description" required rows={2} style={{ padding: 6, width: '100%' }} /></div>
...
export function ComplaintStatusButtons({ complaintId, currentStatus }: { complaintId: string; currentStatus: string }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      ...
      {resolveState?.error ? <span style={{ color: '#b00', fontSize: 11 }}>{resolveState.error}</span> : null}
    </div>
  );
}

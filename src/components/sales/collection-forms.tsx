...
export function CreateCollectionForm({ invoiceId, currencyCode }: { invoiceId: string; currencyCode: string }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 6, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #eee', padding: 8, borderRadius: 4 }}>
      ...
      <div><label style={{ display: 'block', fontSize: 11, color: '#666' }}>Tarih</label><input name="collectionDate" type="date" required style={{ padding: 5 }} /></div>
      <div><label style={{ display: 'block', fontSize: 11, color: '#666' }}>Tutar</label><input name="amount" type="number" step="0.01" required style={{ padding: 5, width: 90 }} /></div>
      <div>
        <label style={{ display: 'block', fontSize: 11, color: '#666' }}>Yöntem</label>

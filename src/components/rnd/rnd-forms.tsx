...
export function CreatePrototypeForm({ projects }: { projects: { id: string; code: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Proje (ops.)</label>
...
export function CreateLabTestForm({ prototypes }: { prototypes: { id: string; prototypeNo: string; name: string }[] }) {
  ...
  return (
    <form action={formAction} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap', border: '1px solid #ddd', padding: 12, borderRadius: 6 }}>
      <div>
        <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Prototip (ops.)</label>
      ...
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Test Adı</label><input name="testName" required style={{ padding: 6, width: 180 }} /></div>
      <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Tarih (ops.)</label><input name="testDate" type="date" style={{ padding: 6 }} /></div>

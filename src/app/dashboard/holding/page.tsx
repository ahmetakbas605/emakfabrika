        ...
        <tbody>
          {holdingCompanies.map((c) => (
            <tr key={c.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', fontFamily: 'monospace', color: '#666' }}>{c.taxId || '—'}</td>
            </tr>
          ))}
          {holdingCompanies.length === 0 ? <tr><td colSpan={2} style={{ padding: '8px', color: '#999' }}>Bu holding'de şirket yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {consolidated.companies.map((c) => (
              <tr key={c.companyId} style={{ borderBottom: '1px solid #eee' }}>

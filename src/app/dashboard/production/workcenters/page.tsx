...
export default async function WorkCentersPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Routing operasyonlarının atandığı makine/hat/grup tanımları.</p>
        ...
        <tbody>
          {workCenters.map((w) => (
            <tr key={w.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666' }}>{w.capacityPerHour ?? '—'}</td>
            </tr>
          ))}
          {workCenters.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: '#999' }}>Henüz iş merkezi yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

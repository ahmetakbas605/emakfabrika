...
export default async function BomPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Aynı kodla yeni bir BOM oluşturmak, önceki versiyonu otomatik olarak eski versiyon işaretler — üretim emirleri her zaman güncel (ACTIVE) versiyonu kullanır.</p>
        ...
        <tbody>
          {boms.map((b) => (
            <tr key={b.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{b.productName}</td>
              <td style={{ padding: '6px 8px' }}>v{b.version}</td>
              <td style={{ padding: '6px 8px', fontWeight: b.status === 'ACTIVE' ? 600 : 400, color: b.status === 'ACTIVE' ? undefined : 'var(--dim-slate)' }}>{STATUS_LABELS[b.status] ?? b.status}</td>
            </tr>
          ))}
          {boms.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz BOM yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
export default async function RoutingPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Opsiyonel — routing tanımlı bir ürünün üretim emri, serbest bırakıldığında her operasyon için otomatik bir iş emri üretir.</p>

      {workCenters.length === 0 ? (
        <p style={{ color: '#b00', fontSize: 13, marginBottom: 20 }}>Önce en az bir İş Merkezi tanımlanmalı.</p>
      ) : (
        ...
        <tbody>
          {routings.map((r) => (
            <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{r.productName}</td>
              <td style={{ padding: '6px 8px' }}>v{r.version}</td>
              <td style={{ padding: '6px 8px', fontWeight: r.status === 'ACTIVE' ? 600 : 400, color: r.status === 'ACTIVE' ? undefined : '#999' }}>{STATUS_LABELS[r.status] ?? r.status}</td>
            </tr>
          ))}
          {routings.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz routing yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
export default async function MesPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Makine/duruş/OEE — Faz 2'nin iş merkezi/üretim emri/iş emri altyapısının üzerine inşa edildi. Bir operasyonun OEE'sini görmek için ilgili Üretim Emri'nin detay sayfasına bakın.</p>

      {openDowntimes.length > 0 ? (
        <>
          <h2 style={{ fontSize: 15, marginBottom: 8, color: 'var(--dim-danger)' }}>Açık Duruşlar ({openDowntimes.length})</h2>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
            <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Makine</th><th style={{ padding: '6px 8px' }}>Neden</th><th style={{ padding: '6px 8px' }}>Başlangıç</th><th style={{ padding: '6px 8px' }}></th></tr></thead>
            <tbody>
              {openDowntimes.map((d) => (
                <tr key={d.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
                  <td style={{ padding: '6px 8px' }}>{d.machineName}</td>
                  <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{d.reasonName}</td>
      ...
      {workCenters.length === 0 ? (
        <p style={{ color: 'var(--dim-danger)', fontSize: 13, marginBottom: 20 }}>Önce Üretim → İş Merkezleri sayfasında en az bir iş merkezi tanımlanmalı.</p>
      ) : (
        ...
        <tbody>
          {machines.map((m) => (
            <tr key={m.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{m.idealCycleTimeSeconds ?? '—'}</td>
              <td style={{ padding: '6px 8px' }}><Link href={`/dashboard/mes/machines/${m.id}`}>OEE Paneli →</Link></td>
            </tr>
          ))}
          {machines.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz makine yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
export default async function DelegationsPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>İzinli olduğunuz dönemde onaylarınız otomatik olarak seçtiğiniz vekile devredilir — yeni başlayan onay örnekleri bunu kullanır, geçmiş onaylar etkilenmez.</p>
        ...
        <tbody>
          {visible.map((d) => (
            <tr key={d.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{new Date(d.startsAt).toLocaleString('tr-TR')}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{new Date(d.endsAt).toLocaleString('tr-TR')}</td>
              <td style={{ padding: '6px 8px', color: d.active ? '#080' : 'var(--dim-slate)' }}>{d.active ? 'Aktif' : 'Pasif'}</td>
              <td style={{ padding: '6px 8px' }}>
                {d.active && d.delegatorUserId === session.id ? <DeactivateDelegationButton delegationId={d.id} /> : null}
              </td>
            </tr>
          ))}
          {visible.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz vekalet yok.</td></tr> : null}
        </tbody>
      </table>

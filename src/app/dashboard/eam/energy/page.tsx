...
export default async function EnergyPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; workCenterId?: string }> }) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Elektrik/doğalgaz/su/buhar/basınçlı hava — dönem bazlı tüketim kaydı. Ürün-başı enerji, bir iş merkezine bağlı sayaçlar için talep üzerine hesaplanır.</p>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Kod</th><th style={{ padding: '6px 8px' }}>Ad</th><th style={{ padding: '6px 8px' }}>Tip</th><th style={{ padding: '6px 8px' }}>Birim</th><th style={{ padding: '6px 8px' }}>İş Merkezi</th></tr></thead>
        <tbody>
          {meters.map((m) => (
            <tr key={m.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{m.code}</td>
              <td style={{ padding: '6px 8px' }}>{m.name}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{ENERGY_TYPE_LABELS[m.energyType]}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{m.unit}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{m.workCenterName ?? '—'}</td>
            </tr>
          ))}
          {meters.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz sayaç yok.</td></tr> : null}
        </tbody>
      </table>

      <h2 style={{ fontSize: 15, marginBottom: 8 }}>Tüketim Kaydet</h2>
      {meters.length === 0 ? (
        <p style={{ color: '#b00', fontSize: 13, marginBottom: 24 }}>Önce en az bir sayaç eklenmeli.</p>
      ) : (
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Sayaç</th><th style={{ padding: '6px 8px' }}>Dönem</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Tüketim</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Maliyet</th></tr></thead>
        <tbody>
          {readings.slice(0, 20).map((r) => (
            <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px' }}>{r.meterName}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{r.periodStart} — {r.periodEnd}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666' }}>{r.consumption} {r.unit}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666' }}>{r.cost ?? '—'}</td>
            </tr>
          ))}
          {readings.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz tüketim kaydı yok.</td></tr> : null}
        </tbody>
      </table>

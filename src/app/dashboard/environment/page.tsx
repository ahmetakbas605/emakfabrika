...
export default async function EnvironmentPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>İSG olay kayıtları için <a href="/dashboard/safety">ayrı sayfa</a>.</p>

      {expiringPermits.length > 0 ? (
        <>
          <h2 style={{ fontSize: 15, marginBottom: 8, color: 'var(--dim-danger)' }}>30 Gün İçinde Sona Erecek İzinler ({expiringPermits.length})</h2>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
            <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>No</th><th style={{ padding: '6px 8px' }}>Tip</th><th style={{ padding: '6px 8px' }}>Son Tarih</th></tr></thead>
            <tbody>
              {expiringPermits.map((p) => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
                  ...
                  <td style={{ padding: '6px 8px', color: 'var(--dim-danger)', fontWeight: 600 }}>{p.expiryDate}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>No</th><th style={{ padding: '6px 8px' }}>Tip</th><th style={{ padding: '6px 8px' }}>Veren Kurum</th><th style={{ padding: '6px 8px' }}>Son Tarih</th><th style={{ padding: '6px 8px' }}>Durum</th></tr></thead>
        <tbody>
          {permits.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              <td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{p.permitNo}</td>
              <td style={{ padding: '6px 8px' }}>{PERMIT_TYPE_LABELS[p.permitType]}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{p.issuingAuthority || '—'}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{p.expiryDate ?? '—'}</td>
              <td style={{ padding: '6px 8px', fontWeight: 600 }}>{PERMIT_STATUS_LABELS[p.status]}</td>
            </tr>
          ))}
          {permits.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz izin yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 24 }}>
        <div>
          ...
          {Object.keys(summary.emissionByType).length === 0 ? <p style={{ color: 'var(--dim-slate)', fontSize: 13 }}>Kayıt yok.</p> : (
        ...
        <div>
          ...
          {Object.keys(summary.wasteByType).length === 0 ? <p style={{ color: 'var(--dim-slate)', fontSize: 13 }}>Kayıt yok.</p> : (
            <ul style={{ fontSize: 13, listStyle: 'none', padding: 0 }}>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Tarih</th><th style={{ padding: '6px 8px' }}>Tip</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Miktar</th><th style={{ padding: '6px 8px' }}>Kaynak</th></tr></thead>
        <tbody>
          {emissions.slice(0, 20).map((e) => (
            <tr key={e.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{e.recordDate}</td>
              <td style={{ padding: '6px 8px' }}>{EMISSION_TYPE_LABELS[e.emissionType]}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{e.quantity} {e.unit}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{e.source || '—'}</td>
            </tr>
          ))}
          {emissions.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz kayıt yok.</td></tr> : null}
        </tbody>
      </table>

      <h2 style={{ fontSize: 15, marginBottom: 8 }}>Atık Kayıtları</h2>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Tarih</th><th style={{ padding: '6px 8px' }}>Tip</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Miktar</th><th style={{ padding: '6px 8px' }}>Bertaraf</th></tr></thead>
        <tbody>
          {wastes.slice(0, 20).map((w) => (
            <tr key={w.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{w.recordDate}</td>
              <td style={{ padding: '6px 8px' }}>{WASTE_TYPE_LABELS[w.wasteType]}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{w.quantity} {w.unit}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{w.disposalCompany || '—'}</td>
            </tr>
          ))}
          {wastes.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz kayıt yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

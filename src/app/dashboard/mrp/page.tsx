...
export default async function MrpPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Satış siparişleri + Minimum stok + Mevcut stok + Açık satın alma + Açık üretim üzerinden net ihtiyaç hesaplar; BOM'u çok seviyeli patlatır. Hiçbir belgeyi OTOMATİK açmaz — her öneri elle dönüştürülür veya iptal edilir.</p>
      ...
      <p style={{ color: '#999', marginBottom: 8, fontSize: 12 }}>Yalnızca Master Data ürünlerine bağlı stok kartları listelenir — MRP yalnızca bunları planlayabilir.</p>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        ...
        <tbody>
          ...
          {stockItems.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: '#999' }}>Master Data'ya bağlı stok kartı yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {runs.map((r) => (
            <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{warehouseNameById.get(r.warehouseId) ?? '—'}</td>
          ...
          {runs.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz MRP koşusu yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

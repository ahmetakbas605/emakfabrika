...
export default async function MrpRunDetailPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>{plannedOrders.length} önerilen kalem. Bir öneriyi dönüştürmek, ilgili modülde (Üretim/Satınalma) GERÇEK bir taslak belge açar.</p>
        ...
        <tbody>
          {plannedOrders.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{SOURCE_LABELS[p.demandSource] ?? p.demandSource}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{p.parentId ? (productNameById.get(p.parentId) ?? '—') : '—'}</td>
          ...
          {plannedOrders.length === 0 ? <tr><td colSpan={7} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Net ihtiyaç bulunamadı — mevcut stok/açık siparişler talebi karşılıyor.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

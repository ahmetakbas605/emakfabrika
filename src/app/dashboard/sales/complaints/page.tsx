...
export default async function ComplaintsPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Satış sonrası şikayet/talep takibi.</p>
        ...
        <tbody>
          {complaints.map((c) => (
            <tr key={c.id} style={{ borderBottom: '1px solid #eee', verticalAlign: 'top' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{PRIORITY_LABELS[c.priority] ?? c.priority}</td>
          ...
          {complaints.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: '#999' }}>Henüz şikayet yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

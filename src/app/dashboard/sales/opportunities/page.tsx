...
export default async function OpportunitiesPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Kazanılan (WON) bir fırsat, Teklif oluşturarak Sipariş'e ilerletilir.</p>
        ...
        <tbody>
          {opportunities.map((o) => (
            <tr key={o.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{o.expectedCloseDate ?? '—'}</td>
          ...
          {opportunities.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz fırsat yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

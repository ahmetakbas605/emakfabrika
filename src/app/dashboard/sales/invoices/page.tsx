...
export default async function SalesInvoicesPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Faturalar genellikle bir siparişin detay sayfasından (kalemlerinden) oluşturulur — bkz. Siparişler.</p>
        ...
        <tbody>
          {invoices.map((i) => (
            <tr key={i.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{i.invoiceDate}</td>
          ...
          {invoices.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz fatura yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
export default async function LeadsPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Kalifiye olan bir aday müşteri, bir cari karta (Müşteri rolüyle) ve bir Fırsata dönüştürülür.</p>
        ...
        <tbody>
          {leads.map((l) => (
            <tr key={l.id} style={{ borderBottom: '1px solid #eee', verticalAlign: 'top' }}>
              <td style={{ padding: '6px 8px' }}>{l.contactName}<br /><span style={{ color: '#999', fontSize: 11 }}>{l.email} {l.phone}</span></td>
              <td style={{ padding: '6px 8px' }}>{l.companyName || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{l.source || '—'}</td>
          ...
          {leads.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz aday müşteri yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
export default async function TendersPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Kapalı zarf teklif toplayan resmi ihale süreçleri. Teklif içeriği, açılış anına kadar gizli tutulur.</p>
        ...
        <tbody>
          {tenders.map((t) => (
            <tr key={t.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{t.bidSubmissionDeadline ? new Date(t.bidSubmissionDeadline).toLocaleString('tr-TR') : '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{t.bidOpeningAt ? new Date(t.bidOpeningAt).toLocaleString('tr-TR') : '—'}</td>
            </tr>
          ))}
          {tenders.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz ihale yok.</td></tr> : null}
        </tbody>
      </table>

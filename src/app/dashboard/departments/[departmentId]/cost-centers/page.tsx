...
export default async function CostCentersPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Muhasebe fişi satırlarına atanmış masraf merkezlerine göre gelir/gider analizi (PDF madde 34).</p>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
        ...
        <tbody>
          {report.map((r) => (
            <tr key={r.costCenterId} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
          ...
          {costCenters.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz masraf merkezi yok.</td></tr> : null}
        </tbody>
      </table>

...
export default async function BudgetsPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Bütçe kalemleri muhasebe fişi üretmez — yalnızca gerçekleşenle karşılaştırılır (PDF madde 35).</p>
        ...
        <tbody>
          {budgets.map((b) => (
            <tr key={b.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{b.status}</td>
            </tr>
          ))}
          {budgets.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz bütçe yok.</td></tr> : null}
        </tbody>
      </table>

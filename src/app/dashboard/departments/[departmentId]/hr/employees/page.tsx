...
export default async function HrEmployeesPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Özlük çekirdeği — personel kartı, iletişim/adres/acil durum kişileri detay sayfasında (İK Mimarisi Faz 0).</p>
          ...
          {employees.map((e) => (
            <tr key={e.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px' }}>
                <Link href={`/dashboard/departments/${departmentId}/hr/employees/${e.id}`} style={{ color: '#111', textDecoration: 'none' }}>{e.firstName} {e.lastName}</Link>
              </td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{e.departmentName ?? '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{e.positionTitle ?? '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{e.hireDate}</td>
              <td style={{ padding: '6px 8px', fontWeight: 600 }}>{STATUS_LABELS[e.employmentStatus] ?? e.employmentStatus}</td>
            </tr>
          ))}
          {employees.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: '#999' }}>Henüz çalışan yok.</td></tr> : null}
        </tbody>
      </table>

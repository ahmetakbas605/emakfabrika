...
export default async function PdksPage({ params, searchParams }: { params: Promise<{ departmentId: string }>; searchParams: Promise<{ date?: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Cihaz Adapter → Raw Punch (silinmez) → Yoklama İşleme akışı — bu faz cihaz entegrasyonu içermiyor, yalnızca Manuel adaptör ile test/backfill (İK Mimarisi raporu §06, Faz 2).</p>
        ...
        <tbody>
          {shiftList.map((s) => (
            <tr key={s.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{s.startTime}–{s.endTime}{s.crossesMidnight ? ' (gece)' : ''}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{s.breakMinutes} dk</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{s.graceMinutes} dk</td>
            </tr>
          ))}
          {shiftList.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz vardiya yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead>
          <tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Çalışan</th><th style={{ padding: '6px 8px' }}>Mevcut Vardiya</th></tr>
        </thead>
        <tbody>
          {employees.map((e) => (
            <tr key={e.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px' }}>{e.firstName} {e.lastName}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{e.shiftName ?? '—'}</td>
            </tr>
          ))}
          {employees.length === 0 ? <tr><td colSpan={2} style={{ padding: '8px', color: '#999' }}>Henüz çalışan yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead>
          <tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Kod</th><th style={{ padding: '6px 8px' }}>Ad</th><th style={{ padding: '6px 8px' }}>Adaptör</th></tr>
        </thead>
        <tbody>
          {devices.map((d) => (
            <tr key={d.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{ADAPTER_LABELS[d.adapterType] ?? d.adapterType}</td>
            </tr>
          ))}
          {devices.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: '#999' }}>Henüz cihaz yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <form method="get" style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 13, color: '#666', marginRight: 8 }}>Tarih:</label>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead>
          <tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Saat</th><th style={{ padding: '6px 8px' }}>Çalışan</th><th style={{ padding: '6px 8px' }}>Cihaz</th><th style={{ padding: '6px 8px' }}>Yön</th><th style={{ padding: '6px 8px' }}>İşlendi</th></tr>
        </thead>
        <tbody>
          {rawPunches.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.deviceName}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{DIRECTION_LABELS[p.direction] ?? p.direction}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.processed ? 'Evet' : 'Hayır'}</td>
            </tr>
          ))}
          {rawPunches.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Bu tarihte kayıt yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      {access.permissions.create && manualDevices.length === 0 ? <p style={{ color: '#999', fontSize: 13, marginBottom: 24 }}>Manuel giriş için önce yukarıdan "Manuel" adaptörlü bir cihaz ekleyin.</p> : null}
        ...
        <tbody>
          {attendanceRecords.map((a) => (
            <tr key={a.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{a.workedMinutes !== null ? `${a.workedMinutes} dk` : '—'}</td>
          ...
          {attendanceRecords.length === 0 ? <tr><td colSpan={7} style={{ padding: '8px', color: '#999' }}>Bu tarih için henüz işlenmiş yoklama kaydı yok.</td></tr> : null}
        </tbody>
      </table>

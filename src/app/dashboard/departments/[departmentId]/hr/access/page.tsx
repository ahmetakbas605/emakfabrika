...
export default async function AccessControlPage({ params, searchParams }: { params: Promise<{ departmentId: string }>; searchParams: Promise<{ date?: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>PDKS'in AYNI cihaz-adapter altyapısını paylaşır — bu faz cihaz entegrasyonu içermiyor, yalnızca Manuel adaptör ile test (İK Mimarisi raporu §06, Faz 4).</p>

      <h2 style={{ fontSize: 15, marginBottom: 8 }}>Bölgeler</h2>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Kod</th><th style={{ padding: '6px 8px' }}>Ad</th><th style={{ padding: '6px 8px' }}>Açıklama</th></tr></thead>
        <tbody>
          {zones.map((z) => (
            <tr key={z.id} style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{z.code}</td><td style={{ padding: '6px 8px' }}>{z.name}</td><td style={{ padding: '6px 8px', color: '#666' }}>{z.description || '—'}</td></tr>
          ))}
          {zones.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: '#999' }}>Henüz bölge yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      {groups.length === 0 ? <p style={{ color: '#999', fontSize: 13 }}>Henüz grup yok.</p> : null}
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Kart No</th><th style={{ padding: '6px 8px' }}>Çalışan</th><th style={{ padding: '6px 8px' }}>Durum</th><th style={{ padding: '6px 8px' }}>İşlem</th></tr></thead>
        <tbody>
          {cards.map((c) => (
          ...
          {cards.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz kart tanımlı değil.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Saat</th><th style={{ padding: '6px 8px' }}>Bölge</th><th style={{ padding: '6px 8px' }}>Çalışan</th><th style={{ padding: '6px 8px' }}>Sonuç</th><th style={{ padding: '6px 8px' }}>Neden</th></tr></thead>
        <tbody>
          {accessLogs.map((l) => (
            <tr key={l.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{formatTime(l.accessAt)}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{l.zoneName}</td>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{l.reason || '—'}</td>
            </tr>
          ))}
          {accessLogs.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Bu tarihte kayıt yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      {access.permissions.create && manualDevices.length === 0 ? <p style={{ color: '#999', fontSize: 13 }}>Manuel test için önce PDKS sayfasından "Manuel" adaptörlü bir cihaz ekleyin (cihaz altyapısı PDKS ile ortak).</p> : null}
    </div>
  );
}

...
export default async function HrLeaveAdminPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Şirket geneli görünüm — çalışanlar kendi taleplerini "İzin Taleplerim"/"Fazla Mesai Taleplerim" sayfalarından oluşturur, onay Onay Kutusu üzerinden yürür.</p>

      <h2 style={{ fontSize: 15, marginBottom: 8 }}>İzin Hak Ediş Tanımlama</h2>
      <p style={{ color: '#666', fontSize: 12, marginBottom: 8 }}>Yasal/kıdem bazlı hesaplama yok — hak ediş İK tarafından doğrudan girilir (madde 33/199 ilkesi).</p>
        ...
        <tbody>
          {leaveRequests.map((r) => (
            <tr key={r.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{LEAVE_TYPE_LABELS[r.leaveType] ?? r.leaveType}</td>
          ...
          {leaveRequests.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: '#999' }}>Henüz izin talebi yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        ...
        <tbody>
          ...
          {overtimeRequests.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz fazla mesai talebi yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

...
function Card({ label, value, danger }: { label: string; value: number | string; danger?: boolean }) {
  return (
    <div style={{ border: '1px solid var(--dim-border-soft)', borderRadius: 6, padding: 16, minWidth: 140 }}>
...
export default async function ItDashboardPage({ params, searchParams }: { params: Promise<{ departmentId: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Tüm sayılar var olan tablolardan canlı hesaplanır — ayrı bir rapor tablosu yok.</p>
      ...
      {searchResult ? (
        <div style={{ border: '1px solid var(--dim-border-soft)', borderRadius: 6, padding: 12, marginBottom: 20, fontSize: 13 }}>
          ...
          {!searchResult.ip && searchResult.assets.length === 0 && searchResult.tickets.length === 0 ? <p style={{ marginTop: 8, color: 'var(--dim-slate)' }}>Sonuç bulunamadı.</p> : null}
        </div>
      ) : null}
      ...
      <table style={{ width: '100%', maxWidth: 400, borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
        <tbody>
          {summary.openTicketsByStatus.map((s) => <tr key={s.status} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}><td style={{ padding: '4px 8px' }}>{TICKET_STATUS_LABELS[s.status] ?? s.status}</td><td style={{ padding: '4px 8px', textAlign: 'right', fontWeight: 600 }}>{s.count}</td></tr>)}
          {summary.openTicketsByStatus.length === 0 ? <tr><td style={{ padding: '4px 8px', color: 'var(--dim-slate)' }}>Açık ticket yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', maxWidth: 400, borderCollapse: 'collapse', fontSize: 13 }}>
        <tbody>
          {summary.assetsByStatus.map((s) => <tr key={s.status} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}><td style={{ padding: '4px 8px' }}>{s.status}</td><td style={{ padding: '4px 8px', textAlign: 'right', fontWeight: 600 }}>{s.count}</td></tr>)}
          {summary.assetsByStatus.length === 0 ? <tr><td style={{ padding: '4px 8px', color: 'var(--dim-slate)' }}>Henüz varlık yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

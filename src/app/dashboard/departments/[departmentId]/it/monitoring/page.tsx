...
export default async function MonitoringPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Gerçek SNMP/ICMP toplayıcı henüz yok (NetworkDiscoveryAdapter Null stub, Faz 13 tamamlanınca bağlanacak) — ölçümler bugün elle girilir.</p>

      <h2 style={{ fontSize: 16, marginBottom: 8 }}>İzleme Hedefleri</h2>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Varlık</th><th style={{ padding: '6px 8px' }}>Tür</th><th style={{ padding: '6px 8px' }}>Aralık</th></tr></thead>
        <tbody>
          {targets.map((t) => <tr key={t.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}><td style={{ padding: '6px 8px' }}>{t.assetTag} — {t.assetName}</td><td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{t.targetType}</td><td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{t.intervalSeconds}sn</td></tr>)}
          {targets.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz izleme hedefi yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 8 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Varlık</th><th style={{ padding: '6px 8px' }}>Önem</th><th style={{ padding: '6px 8px' }}>Mesaj</th><th style={{ padding: '6px 8px' }}>Durum</th><th style={{ padding: '6px 8px' }}>Grup</th><th style={{ padding: '6px 8px' }}>İşlem</th></tr></thead>
        <tbody>
          {alerts.map((a) => (
            <tr key={a.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{a.status}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-slate)', fontFamily: 'monospace', fontSize: 11 }}>{a.correlationGroupId?.slice(0, 8)}</td>
          ...
          {alerts.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz alert yok.</td></tr> : null}
        </tbody>
      </table>

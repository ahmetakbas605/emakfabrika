...
export default async function NetworkDiagramListPage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Her kaydetme yeni bir versiyon açar — eski versiyon silinmez (NETWORK.md §3).</p>

      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Ad</th><th style={{ padding: '6px 8px' }}>Durum</th></tr></thead>
        <tbody>
          {diagrams.map((d) => (
            <tr key={d.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{d.currentVersionId ? 'Kayıtlı versiyon var' : 'Henüz boş'}</td>
            </tr>
          ))}
          {diagrams.length === 0 ? <tr><td colSpan={2} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz diyagram yok.</td></tr> : null}
        </tbody>
      </table>

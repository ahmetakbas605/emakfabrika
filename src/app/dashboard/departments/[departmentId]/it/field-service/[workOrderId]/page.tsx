...
export default async function WorkOrderDetailPage({ params }: { params: Promise<{ departmentId: string; workOrderId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Ticket durumu: <b>{workOrder.status}</b></p>

      <h2 style={{ fontSize: 16, marginBottom: 8 }}>Varış</h2>
      {workOrder.arrivedAt ? (
        <p style={{ marginBottom: 20, fontSize: 13, color: '#666' }}>Varış zamanı: {new Date(workOrder.arrivedAt).toLocaleString('tr-TR')} — konum: {workOrder.arrivalLatitude}, {workOrder.arrivalLongitude}</p>
      ) : access.permissions.update ? (
        <div style={{ marginBottom: 20 }}><MarkArrivedForm departmentId={departmentId} workOrderId={workOrderId} /></div>
      ) : <p style={{ marginBottom: 20, fontSize: 13, color: '#999' }}>Henüz varış kaydedilmedi.</p>}
      ...
      {checklist ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 20 }}>
          ...
          {checklist.items.length === 0 ? <p style={{ color: '#999', fontSize: 13 }}>Checklist boş.</p> : null}
        </div>
      ) : access.permissions.update ? (
      ...
      ) : <p style={{ marginBottom: 20, fontSize: 13, color: '#999' }}>Henüz checklist eklenmedi.</p>}
        ...
        <tbody>
          {parts.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{Number(p.unitCost).toFixed(2)}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.billable ? 'Evet' : 'Hayır'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.consumedByName}</td>
            </tr>
          ))}
          {parts.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz malzeme tüketimi yok.</td></tr> : null}
        </tbody>
      </table>
      {access.permissions.update ? (
      ...
      {workOrder.signatureNote ? (
        <p style={{ fontSize: 13, color: '#666' }}>{workOrder.customerName}: &quot;{workOrder.signatureNote}&quot;</p>
      ) : access.permissions.update ? (
        <SignatureForm departmentId={departmentId} workOrderId={workOrderId} />
      ) : <p style={{ fontSize: 13, color: '#999' }}>Henüz onay alınmadı.</p>}
    </div>
  );
}

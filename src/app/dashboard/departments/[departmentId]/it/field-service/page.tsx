...
export default async function FieldServicePage({ params }: { params: Promise<{ departmentId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Bir work order, saha (FIELD_SERVICE) tipi ticket&apos;ın konum/checklist/malzeme ekli hâlidir (FIELD-SERVICE.md §1).</p>
        ...
        <tbody>
          {workOrders.map((w) => (
            <tr key={w.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{w.arrivedAt ? new Date(w.arrivedAt).toLocaleString('tr-TR') : '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{w.customerName ?? '—'}</td>
            </tr>
          ))}
          {workOrders.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz work order yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 12 }}>
            ...
            <tbody>
              ...
              {templates.length === 0 ? <tr><td colSpan={2} style={{ padding: '8px', color: '#999' }}>Henüz şablon yok.</td></tr> : null}
            </tbody>
          </table>

...
export default async function TicketDetailPage({ params }: { params: Promise<{ departmentId: string; ticketId: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>{ticket.category || 'Kategorisiz'} · {ticket.priority} · Talep eden: {ticket.requestedByName}</p>

      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
        <span style={{ fontWeight: 700, padding: '4px 10px', border: '1px solid var(--dim-border-soft)', borderRadius: 4 }}>{STATUS_LABELS[ticket.status] ?? ticket.status}</span>
        {ticket.slaDueAt ? <span style={{ color: 'var(--dim-on-surface-variant)', fontSize: 13 }}>SLA: {new Date(ticket.slaDueAt).toLocaleString('tr-TR')}</span> : null}
        {escalations.length > 0 ? <span style={{ color: 'var(--dim-danger)', fontSize: 13, fontWeight: 600 }}>Eskale edildi — Seviye {escalations[0].level} ({escalations[0].roleName ?? escalations[0].escalatedToRoleCode})</span> : null}
      </div>
        ...
        <tbody>
          {assignments.map((a) => (
            <tr key={a.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{a.role === 'LEADER' ? 'Sorumlu' : 'Yardımcı'}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{new Date(a.assignedAt).toLocaleString('tr-TR')}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{a.unassignedAt ? new Date(a.unassignedAt).toLocaleString('tr-TR') : '—'}</td>
            </tr>
          ))}
          {assignments.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz atama yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
        {timeline.map((e, i) => (
          <div key={i} style={{ fontSize: 13, borderLeft: '2px solid var(--dim-border)', paddingLeft: 10 }}>
            <span style={{ color: 'var(--dim-slate)', fontSize: 11 }}>{new Date(e.at).toLocaleString('tr-TR')}</span>
        ...
        {timeline.length === 0 ? <p style={{ color: 'var(--dim-slate)', fontSize: 13 }}>Henüz kayıt yok.</p> : null}
      </div>

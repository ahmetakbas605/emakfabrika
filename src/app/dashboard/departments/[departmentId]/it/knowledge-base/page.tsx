...
export default async function KnowledgeBasePage({ params, searchParams }: { params: Promise<{ departmentId: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Yaygın çözümler ve prosedürler — basit metin araması yeterli (tam metin arama motoru gereksiz bir soyutlama).</p>
      ...
      <ul style={{ marginBottom: 20, fontSize: 13 }}>
        ...
        {categories.length === 0 ? <li style={{ color: '#999' }}>Henüz kategori yok.</li> : null}
      </ul>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 12 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Başlık</th><th style={{ padding: '6px 8px' }}>Kategori</th><th style={{ padding: '6px 8px' }}>Yazar</th><th style={{ padding: '6px 8px' }}>Görüntülenme</th></tr></thead>
        <tbody>
          {articles.map((a) => (
            <tr key={a.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px' }}><Link href={`/dashboard/departments/${departmentId}/it/knowledge-base/${a.id}`}>{a.title}</Link></td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{a.categoryName || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{a.authorName}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{a.viewCount}</td>
            </tr>
          ))}
          {articles.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz makale yok.</td></tr> : null}
        </tbody>
      </table>

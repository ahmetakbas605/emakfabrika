...
export default async function SafetyPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Eğitim/PDKS zaten İK modülünde tutuluyor — bu sayfa yalnızca HR-dışı olay/kaza kayıtlarını kapsar. Çevre kayıtları için <a href="/dashboard/environment">ayrı sayfa</a>.</p>
        ...
        <tbody>
          {incidents.map((i) => (
            <tr key={i.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{i.incidentDate}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{i.employeeName ? `${i.employeeName} ${i.employeeLastName}` : '—'}</td>
          ...
          {incidents.length === 0 ? <tr><td colSpan={7} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz olay kaydı yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

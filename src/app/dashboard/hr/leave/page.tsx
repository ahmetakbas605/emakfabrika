...
export default async function LeavePage() {
  ...
  if (!session.employeeId) {
    return (
      <div style={{ padding: '2rem' }}>
        ...
        <p style={{ color: 'var(--dim-on-surface-variant)' }}>ERP hesabınız bir özlük kaydına bağlı değil — izin talebi oluşturabilmek için İK ile iletişime geçin.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Onay motoru genel — bkz. Onay Kutusu. İzin türü "Devamsızlık" plansız/mazeretsiz gaybubeti sonradan bildirmek için kullanılır.</p>
      ...
      {balances.length > 0 ? (
        <>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--dim-border)' }}><th style={{ padding: '6px 8px' }}>Tür</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Hak Ediş</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Kullanılan</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Kalan</th></tr>
            </thead>
            <tbody>
              {balances.map((b) => (
                <tr key={b.leaveType} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
                  ...
                  <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{b.usedDays}</td>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
        ...
        <tbody>
          ...
          {requests.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz izin talebiniz yok.</td></tr> : null}
        </tbody>
      </table>

...
function Stat({ label, value, accent }: { label: string; value: string | number; accent?: string }) {
  return (
    <div style={{ border: '1px solid #ddd', borderRadius: 6, padding: '10px 16px', minWidth: 140 }}>
  ...
  return (
    <div style={{ padding: '2rem' }}>
        ...
        <div>
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 13 }}>
            <tbody>
              ...
              {factory.productionOrdersByStatus.length === 0 ? <tr><td style={{ color: '#999' }}>Kayıt yok</td></tr> : null}
            </tbody>
          </table>
        </div>
        <div>
          ...
          <table style={{ borderCollapse: 'collapse', fontSize: 13 }}>
            <tbody>
              ...
              {factory.eamAssetsByStatus.length === 0 ? <tr><td style={{ color: '#999' }}>Kayıt yok</td></tr> : null}
            </tbody>
          </table>
        </div>
      ...
      {cfo.fxExposure.length > 0 ? (
        <div style={{ overflowX: 'auto', marginBottom: 24 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Hesap</th><th style={{ padding: '6px 8px' }}>Para Birimi</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Bakiye</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Gerçekleşmemiş K/Z</th></tr></thead>
            <tbody>
      ...
      <p style={{ fontSize: 13, marginBottom: 24 }}>
        {itDepartment ? <a href={`/dashboard/departments/${itDepartment.id}/it/dashboard`}>BT Kontrol Paneline git</a> : <span style={{ color: '#999' }}>Şirkette BT departmanı tanımlı değil.</span>}
      </p>

      <h2 style={{ fontSize: 16, marginBottom: 8 }}>Alert Center ({alerts.length})</h2>
      <div style={{ overflowX: 'auto', marginBottom: 24 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Önem</th><th style={{ padding: '6px 8px' }}>Modül</th><th style={{ padding: '6px 8px' }}>Açıklama</th></tr></thead>
          <tbody>
            {sortedAlerts.map((a) => (
              <tr key={`${a.module}-${a.itemType}-${a.id}`} style={{ borderBottom: '1px solid #eee' }}>
            ...
            {alerts.length === 0 ? <tr><td colSpan={3} style={{ padding: '8px', color: '#999' }}>Açık uyarı yok.</td></tr> : null}
          </tbody>
        </table>
      </div>
      ...
      <form method="get" style={{ display: 'flex', gap: 8, alignItems: 'flex-end', marginBottom: 12 }}>
        <div><label style={{ display: 'block', fontSize: 12, color: '#666' }}>Gün İçinde</label><input name="withinDays" type="number" min={1} defaultValue={withinDays} style={{ padding: 6, width: 100 }} /></div>
        <button type="submit" style={{ padding: '7px 14px', cursor: 'pointer' }}>Uygula</button>
      </form>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Modül</th><th style={{ padding: '6px 8px' }}>Tür</th><th style={{ padding: '6px 8px' }}>Açıklama</th><th style={{ padding: '6px 8px' }}>Son Tarih</th><th style={{ padding: '6px 8px', textAlign: 'right' }}>Kalan Gün</th></tr></thead>
          <tbody>
            {expiring.map((e) => (
              <tr key={`${e.module}-${e.itemType}-${e.id}`} style={{ borderBottom: '1px solid #eee' }}>
            ...
            {expiring.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Bu pencerede sona erecek kayıt yok.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>

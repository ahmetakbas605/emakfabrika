...
export default async function RndPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Ar-Ge projeleri mevcut <a href="/dashboard/projects">Proje Yönetimi</a> modülünü doğrudan kullanır — burada yalnızca prototip/laboratuvar testleri var.</p>
        ...
        <tbody>
          {prototypes.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{p.projectName ?? '—'}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{p.version}</td>
          ...
          {prototypes.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz prototip yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {labTests.map((t) => (
            <tr key={t.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{t.prototypeName ?? '—'}</td>
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{t.testDate ?? '—'}</td>
          ...
          {labTests.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz laboratuvar testi yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

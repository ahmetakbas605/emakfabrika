  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <form method="get" style={{ display: 'flex', gap: 8, alignItems: 'flex-end', marginBottom: 16 }}>
        <div>
          <label style={{ display: 'block', fontSize: 12, color: '#666' }}>Olay Türü</label>
          ...
          <tbody>
            {events.map((e) => (
              <tr key={e.id} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={{ padding: '6px 8px', fontFamily: 'monospace', color: '#666' }}>{e.entityId ?? '—'}</td>
                <td style={{ padding: '6px 8px', fontFamily: 'monospace', color: '#666' }}>{e.payload ? JSON.stringify(e.payload) : '—'}</td>
              </tr>
            ))}
            {events.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz olay yok.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>

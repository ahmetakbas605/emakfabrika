...
export default async function FleetExpensesPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; vehicleId?: string }> }) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: 'var(--dim-on-surface-variant)', marginBottom: 20, fontSize: 13 }}>Yakıt verimliliği, kilometre okumaları arasındaki farktan talep üzerine hesaplanır — saklanan bir değer değil.</p>
        ...
        <tbody>
          {expenses.slice(0, 20).map((e) => (
            <tr key={e.id} style={{ borderBottom: '1px solid var(--dim-border-soft)' }}>
              ...
              <td style={{ padding: '6px 8px', color: 'var(--dim-on-surface-variant)' }}>{e.expenseDate}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{e.amount}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{e.quantity ?? '—'}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: 'var(--dim-on-surface-variant)' }}>{e.odometerKm ?? '—'}</td>
            </tr>
          ))}
          {expenses.length === 0 ? <tr><td colSpan={6} style={{ padding: '8px', color: 'var(--dim-slate)' }}>Henüz gider kaydı yok.</td></tr> : null}
        </tbody>
      </table>

...
export default async function FleetPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>IT'nin mevcut bakım plan/iş emri motoru üzerine kuruldu. Yakıt/HGS giderleri <Link href="/dashboard/fleet/expenses">ayrı sayfada</Link>.</p>

      {expiringDocs.length > 0 ? (
        <>
          <h2 style={{ fontSize: 15, marginBottom: 8, color: '#b00' }}>30 Gün İçinde Sona Erecek Belgeler ({expiringDocs.length})</h2>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 20 }}>
            <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Plaka</th><th style={{ padding: '6px 8px' }}>Belge</th><th style={{ padding: '6px 8px' }}>Son Tarih</th></tr></thead>
            <tbody>
              {expiringDocs.map((d, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                  <td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{d.plateNo}</td>
                  <td style={{ padding: '6px 8px', color: '#666' }}>{d.detail}</td>
                  <td style={{ padding: '6px 8px', color: '#b00', fontWeight: 600 }}>{d.expiryDate}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <tbody>
          {vehicles.map((v) => (
            <tr key={v.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{FUEL_LABELS[v.fuelType]}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{v.registrationExpiryDate ?? '—'}</td>
              <td style={{ padding: '6px 8px', fontWeight: 600, color: v.status === 'UNDER_MAINTENANCE' ? '#a60' : v.status === 'ACTIVE' ? '#080' : '#b00' }}>{STATUS_LABELS[v.status]}</td>
            </tr>
          ))}
          {vehicles.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz araç yok.</td></tr> : null}
        </tbody>
      </table>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 24 }}>
        <thead><tr style={{ textAlign: 'left', borderBottom: '2px solid #333' }}><th style={{ padding: '6px 8px' }}>Plaka</th><th style={{ padding: '6px 8px' }}>Poliçe No</th><th style={{ padding: '6px 8px' }}>Şirket</th><th style={{ padding: '6px 8px' }}>Kapsam</th><th style={{ padding: '6px 8px' }}>Bitiş</th></tr></thead>
        <tbody>
          {insurances.map((i) => (
            <tr key={i.id} style={{ borderBottom: '1px solid #eee' }}>
              <td style={{ padding: '6px 8px', fontFamily: 'monospace' }}>{i.plateNo}</td>
              <td style={{ padding: '6px 8px' }}>{i.policyNo}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{i.provider || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{i.coverageType || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{i.endDate}</td>
            </tr>
          ))}
          {insurances.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz poliçe yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {plans.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666', fontFamily: 'monospace' }}>{p.plateNo}</td>
        ...
        <tbody>
          {fleetWorkOrders.map((w) => (
            <tr key={w.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666', fontFamily: 'monospace' }}>{w.plateNo}</td>
          ...
          {fleetWorkOrders.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz bakım iş emri yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

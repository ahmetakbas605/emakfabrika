...
export default async function ProjectsPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Proje/görev/milestone/bütçe/hakediş. Satın Alma talepleri opsiyonel olarak bir projeye bağlanabilir.</p>
        ...
        <tbody>
          {projects.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.managerName ?? '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.departmentName ?? '—'}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666' }}>{p.budgetAmount ?? '—'}</td>
          ...
          {projects.length === 0 ? <tr><td colSpan={7} style={{ padding: '8px', color: '#999' }}>Henüz proje yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

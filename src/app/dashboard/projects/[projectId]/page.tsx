...
export default async function ProjectDetailPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>{project.description || 'Açıklama yok.'}</p>
        ...
        <tbody>
          {tasks.map((t) => (
            <tr key={t.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{t.assignedToName ?? '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{t.dueDate ?? '—'}</td>
          ...
          {tasks.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz görev yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {milestones.map((m) => (
            <tr key={m.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{m.targetDate}</td>
          ...
          {milestones.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz milestone yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {payments.map((p) => (
            <tr key={p.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{p.periodStart} — {p.periodEnd}</td>
              <td style={{ padding: '6px 8px', textAlign: 'right', color: '#666' }}>{p.amount}</td>
          ...
          {payments.length === 0 ? <tr><td colSpan={5} style={{ padding: '8px', color: '#999' }}>Henüz hakediş yok.</td></tr> : null}
        </tbody>
      </table>
    </div>
  );

        ...
        <tbody>
          {addresses.map((a) => (
            <tr key={a.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{a.city} {a.district}</td>
            </tr>
          ))}
          {addresses.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz adres yok.</td></tr> : null}
        </tbody>
      </table>
        ...
        <tbody>
          {contacts.map((c) => (
            <tr key={c.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{c.title || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{c.email || '—'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{c.phone || '—'}</td>
            </tr>
          ))}
          {contacts.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz yetkili yok.</td></tr> : null}
        </tbody>
      </table>

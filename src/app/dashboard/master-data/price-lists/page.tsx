...
export default async function PriceListsPage() {
  ...
  return (
    <div>
      ...
      <p style={{ color: '#666', marginBottom: 20, fontSize: 13 }}>Genel liste veya müşteriye özel fiyatlandırma (madde 30-31).</p>
        ...
        <tbody>
          {lists.map((l) => (
            <tr key={l.id} style={{ borderBottom: '1px solid #eee' }}>
              ...
              <td style={{ padding: '6px 8px', color: '#666' }}>{l.partyName ?? 'Genel'}</td>
              <td style={{ padding: '6px 8px', color: '#666' }}>{l.validFrom ?? '—'} → {l.validTo ?? '—'}</td>
            </tr>
          ))}
          {lists.length === 0 ? <tr><td colSpan={4} style={{ padding: '8px', color: '#999' }}>Henüz fiyat listesi yok.</td></tr> : null}
        </tbody>
      </table>
